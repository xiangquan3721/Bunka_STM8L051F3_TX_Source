...
void EXTI_PORTA1(void){
 // if(PIN_KEY_LOGIN==0)ADF7021_DATA_tx=!ADF7021_DATA_tx;
  if(FLAG_APP_TX==1){
	...
	if(txphase_Repeat>=4){FLAG_APP_TX=0;PIN_TX_LED=0;ADF7021_POWER=FG_NOT_allow_out;ADF7021_DATA_tx=0;}
    }    
  }
  EXTI_SR1_bit.P4F=1;
  //EXTI_SR2_bit.PBF=1;
...
void SendTxData(void)
{
  UINT8 i,Bits=20;
       ...
       m_RFNormalBuf[0]=0xFF;
       for(i=1;i<=Bits;i++)m_RFNormalBuf[i]=0x55;
       m_RFNormalBuf[Bits+1]=0x15;
       PIN_TX_LED=1;
    //    if(m_RegMode==0){
	//  txphase_end=(Bits+15)*8;
	//  SetTxData(Bits+2,ID_data,Control_code);
    //      m_RFNormalBuf[Bits+14]=0xFF;
    //    }
    //    else {
	//  txphase_end=(Bits+27)*8;
	//  SetTxData(Bits+2,ID_data,0x80);
	//  if(m_RegMode==1)SetTxData(Bits+14,ID_data_add,0xFF);    //"1"是追加
	//  else SetTxData(Bits+14,ID_data_add,0);    //"2"是抹消
    //      m_RFNormalBuf[Bits+26]=0xFF;
    //    }

		 txphase_end=(Bits+31)*8;
		 TX_DataLoad_HighSpeed(ID_data.IDL,Uart_Struct_DATA_Packet_Contro, &m_RFNormalBuf[Bits+2]);
		 m_RFNormalBuf[Bits+30]=0xFF;
