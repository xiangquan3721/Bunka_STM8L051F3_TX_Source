...
void EXTI_PORTA1(void){
 // if(PIN_KEY_LOGIN==0)ADF7021_DATA_tx=!ADF7021_DATA_tx;
  if(FLAG_APP_TX==1){
	...
	if(txphase_Repeat>=2){FLAG_APP_TX=0;PIN_TX_LED=0;ADF7021_POWER=FG_NOT_allow_out;ADF7021_DATA_tx=0;}
    }    
  }
  EXTI_SR1_bit.P4F=1;
  //EXTI_SR2_bit.PBF=1;
...
//       FLAG_APP_TX=1;
       ...
       m_RFNormalBuf[0]=0xFF;
       for(i=1;i<=6;i++)m_RFNormalBuf[i]=0x55;
       m_RFNormalBuf[7]=0x15;
       PIN_TX_LED=1;
       if(m_RegMode==0){
	 txphase_end=160;
	 SetTxData(8,ID_data,Control_code);
       }
       else {
	 txphase_end=256;
	 SetTxData(8,ID_data,0x80);
	 if(m_RegMode==1)SetTxData(20,ID_data_add,0xFF);    //"1"是追加
	 else SetTxData(20,ID_data_add,0);    //"2"是抹消
       }
       txphase=0;
       txphase_Repeat=0;
       ID_INT_CODE=0;
