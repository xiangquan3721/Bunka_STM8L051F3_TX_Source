...
void START_AD_SAMPLER(void){	// 启动AD转换（中断模式）
        ...
        //ADC1_CR3_SMTP2= 1;
        ADC1_CR3_SMTP2= 7;     //384 ADC clock cycles
        
        ADC1_SQR1_CHSEL_S28=1;
        
        ADC1_CR2_PRESC = 1;//时钟两分频
...
void ADC2_EOC_INT(void){
	UINT16 Tp_data=0;
        ...
        Tp_data = AD_DATA_BUFF[1]*256+AD_DATA_BUFF[0];
        if(Tp_data > 50)//太小有问题
        {
	   RAM_BAT_SUM += Tp_data;
           RAM_BAT_CNT++;
...
void AD_control(void){
     ...
     if(RAM_BAT_CNT>=30){
		...
		RAM_BAT_SUM = 0;		// clean sum
                RAM_BAT_CNT = 0;
                AD_Flag =1;
                 if(BAT_Voltage_value>2400)
               {
                   BAT_out=0;
                }
               if((BAT_Voltage_value<=2400)&&(BAT_Voltage_value>2200))
               {
                  BAT_out=1;
               }
               if(BAT_Voltage_value<=2200) 
               {
                  BAT_out=2;
               }
              // PIN_LED = 1;
