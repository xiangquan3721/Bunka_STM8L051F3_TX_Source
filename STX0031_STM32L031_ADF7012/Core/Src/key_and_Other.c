...
//	}
   ...
   if(FG_1ms){
    ...
    if(TIME_BEEP_on){
      --TIME_BEEP_on;
      if(FG_beep_on==0){FG_beep_on=1;FG_beep_off=0;MX_TIM2_Init();//BEEP_CSR2_BEEPEN=1; //2015.3.11修正
                        if(FG_LED_on)
													#ifndef MODEL_TEST
      ...
      --TIME_BEEP_off;
      if(FG_beep_off==0){FG_beep_off=1;FG_beep_on=0;HAL_TIM_OC_Stop(&htim2,TIM_CHANNEL_2);//BEEP_CSR2_BEEPEN=0;  //2015.3.11修正
                        if(FG_LED_on)
													#ifndef MODEL_TEST
...
void	_KeyInTx( void )
{
	...
	/*	Registration sw		*/
	_SwIn( HAL_GPIO_ReadPin(OPEN_GPIO_Port,OPEN_Pin) ) ;
	/*	Auto Tx Start sw		*/
	_SwIn( HAL_GPIO_ReadPin(STOP_GPIO_Port,STOP_Pin) ) ;
	/*	Auto Tx Stop sw		*/
	_SwIn( HAL_GPIO_ReadPin(CLOSE_GPIO_Port,CLOSE_Pin) ) ;
	/*	Auto Tx Vent. sw		*/
	_SwIn( HAL_GPIO_ReadPin(VENT_GPIO_Port,VENT_Pin) ) ;	
	//_SwIn( 1 ) ;
	/*	Auto Tx Reg. sw		*/
	_SwIn( HAL_GPIO_ReadPin(LOGIN_GPIO_Port,LOGIN_Pin) ) ;	
...
{
  UINT8 time_key;
  if((TB_sum_5s<64)&&(FG_PWRON==1)&&(TB_5s<25)){      //计算剩余的时间，总共时间不能超过69，以25为间隔。
    time_key=25-TB_5s;
    TB_sum_5s=TB_sum_5s+time_key;
    if((64-TB_sum_5s)>=27)TB_5s=25;
...
extern uint8_t Command_S,Command_FM,Command_END;
...
void test_mode_control(void)
{
 ...
 //while(HAL_GPIO_ReadPin(TEST_MODE_GPIO_Port,TEST_MODE_Pin)==GPIO_PIN_RESET){  
	while(1){
  HAL_IWDG_Refresh(&hiwdg); // Service the WDT 
  if(((HAL_GPIO_ReadPin(OPEN_GPIO_Port,OPEN_Pin)==GPIO_PIN_RESET)&&(FG_KEY_OPEN==0))||(Command_S==1)){
    Command_S = 0;
		FG_KEY_OPEN=1;
    dd_set_ADF7021_Power_on();
    dd_set_TX_mode();
  ...
  if(HAL_GPIO_ReadPin(OPEN_GPIO_Port,OPEN_Pin)==GPIO_PIN_SET)FG_KEY_OPEN=0;
  
   if(((HAL_GPIO_ReadPin(STOP_GPIO_Port,STOP_Pin)==GPIO_PIN_RESET)&&(FG_KEY_STOP==0))||(Command_END==1)){
    Command_END = 0;
		FG_KEY_STOP=1;
    //ADF7021_CE = 0;
    HAL_GPIO_WritePin(ADF7021_CE_GPIO_Port,ADF7021_CE_Pin,GPIO_PIN_RESET);
    FG_test_mode=0;
    HAL_GPIO_WritePin(TXDATA_GPIO_Port,TXDATA_Pin,GPIO_PIN_RESET);
  }
  if(HAL_GPIO_ReadPin(STOP_GPIO_Port,STOP_Pin)==GPIO_PIN_SET)FG_KEY_STOP=0; 
  
  if(((HAL_GPIO_ReadPin(CLOSE_GPIO_Port,CLOSE_Pin)==GPIO_PIN_RESET)&&(FG_KEY_CLOSE==0))||(Command_FM==1)){
    Command_FM = 0;
		FG_KEY_CLOSE=1;
		exit_init();
    dd_set_ADF7021_Power_on();
  ...
  if(HAL_GPIO_ReadPin(CLOSE_GPIO_Port,CLOSE_Pin)==GPIO_PIN_SET)FG_KEY_CLOSE=0;  
  
	
  if((HAL_GPIO_ReadPin(EXTI_TXCLK_GPIO_Port,EXTI_TXCLK_Pin)==GPIO_PIN_SET)&&(FG_test_mode==1)&&(FG_test1==0)){
     HAL_GPIO_TogglePin(TXDATA_GPIO_Port,TXDATA_Pin);
     FG_test1=1;
  }
  if(HAL_GPIO_ReadPin(EXTI_TXCLK_GPIO_Port,EXTI_TXCLK_Pin)==GPIO_PIN_RESET)FG_test1=0;	 
