...
{
 if(GPIO_Pin == EXTI_TXCLK_Pin)
 {
	// if(PIN_KEY_LOGIN==0)ADF7021_DATA_tx=!ADF7021_DATA_tx;
  if(FLAG_APP_TX==1){
    if(txphase%8==0)ID_INT_CODE=m_RFNormalBuf[txphase/8];
    if	(ID_INT_CODE & 0x80)HAL_GPIO_WritePin(TXDATA_GPIO_Port,TXDATA_Pin,GPIO_PIN_SET);
    else HAL_GPIO_WritePin(TXDATA_GPIO_Port,TXDATA_Pin,GPIO_PIN_RESET);
    ID_INT_CODE<<=1;
//		if	(ID_INT_CODE & 0x01)HAL_GPIO_WritePin(TXDATA_GPIO_Port,TXDATA_Pin,GPIO_PIN_SET);
//    else HAL_GPIO_WritePin(TXDATA_GPIO_Port,TXDATA_Pin,GPIO_PIN_RESET);
//    ID_INT_CODE>>=1;
    txphase++;
     //if(txphase>=224){
...
void SetTxData(UINT8 count_set ,uni_rom_id ID_data_set,UINT8 Control_code_set)
{
	...
	m_RFNormalBuf[count_set++] = unii.uc[1] ;
	m_RFNormalBuf[count_set++] = unii.uc[0] ;
  	//m_RFNormalBuf[23] = 0x59;
		//m_RFNormalBuf[24] = 0xa9;
		//m_RFNormalBuf[25] = 0x95;
		//m_RFNormalBuf[26] = 0x66;
}
