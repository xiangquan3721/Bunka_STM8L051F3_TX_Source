...
void WriteByteToFLASH(ulong Addr, uchar Dat){	// 写入一字节到eeprom
	...
	HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_BYTE ,Addr,Dat);
}
////------------------------------------------------
...
////	for (i=0;i<16;i++)
////	   HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_BYTE ,addr_eeprom_sys+i,eeprom_sys_buff);
...
void eeprom_sys_load(void){
  ...
  rom_KeyOpt= *(uint8_t *)( addr_eeprom_sys+0x70 );
  if(rom_KeyOpt>1)rom_KeyOpt=0;
  
  
  Init_ID_data.IDB[3] =0;  
  Init_ID_data.IDB[2] = *(uint8_t *)( addr_eeprom_sys+0x10 );
  Init_ID_data.IDB[1] = *(uint8_t *)( addr_eeprom_sys+0x11 );
  Init_ID_data.IDB[0] = *(uint8_t *)( addr_eeprom_sys+0x12 );  
  if(Init_ID_data.IDL==0xFFFFFF)Init_ID_data.IDL=0;
  ID_data.IDL=Init_ID_data.IDL;
  //ID_data.IDL=13475495;
  
  for (i=0;i<4;i++)
  {
    j=0x20+i*4;
    ROM_adf7012_value[i].byte[0]= *(uint8_t *)( addr_eeprom_sys+j );
    ROM_adf7012_value[i].byte[1]= *(uint8_t *)( addr_eeprom_sys+j+1 );
    ROM_adf7012_value[i].byte[2]= *(uint8_t *)( addr_eeprom_sys+j+2 );
    ROM_adf7012_value[i].byte[3]= *(uint8_t *)( addr_eeprom_sys+j+3 );
...
void KeyOpt_EEPROM_write(void)
{
  ...
  HAL_FLASHEx_DATAEEPROM_Unlock();
	HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_BYTE ,addr_eeprom_sys+0x70,rom_KeyOpt);
	HAL_FLASHEx_DATAEEPROM_Lock();
