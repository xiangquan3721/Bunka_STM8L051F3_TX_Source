...
//	}
   ...
   if(FG_1ms){
    ...
    if(TIME_BEEP_on)
    {
        ...
        if(FG_beep_on==0)
        {
            FG_beep_on=1;FG_beep_off=0;Beep_On();//BEEP_CSR2_BEEPEN=1; //2015.3.11����
    ...
    else if(TIME_BEEP_off)
    {
        ...
        if(FG_beep_off==0)
        {
            FG_beep_off=1;FG_beep_on=0;Beep_Off();//BEEP_CSR2_BEEPEN=0;  //2015.3.11����
...
//    else {ADC_read();FG_BAT_value=0;}
    _KeyInTx();
    if(FG_10s==1) return;   // 2015.1.31����3
    _RegistrationMode();
    _DupliFuncSetMode();
    //ADC2_EOC_INT();
...
void time_control(void)
{
    if(FG_100ms){
        FG_100ms=0; 
        if(TIME_2s_RestTX)--TIME_2s_RestTX;    //2015.4.13����    
        if(FG_PWRON==1){
...
void	_KeyInTx( void )
{
	...
	{												// Different with the last state
		...
		_ClearSpecialMultiKeyState() ;				// Clear key continue state
                TIME_Once_twice_switch=5000;   //2015.1.31����4
                TIME_10s=10100;  //2015.1.31����3
		return ;
	}
...
//		}
	    //if(BAT_value>1000){
           if(TIME_Once_twice_switch)--TIME_Once_twice_switch;    //2015.1.31����4
           if(TIME_10s)--TIME_10s;    //2015.1.31����3
	   //dd_set_ADF7021_Power_on_Init();
//            dd_set_ML7345D_Power_on_Init();
//		dd_set_CMT2300A_Power_on_Init();
           if((BAT_out==1)||(FG_BAT)||(TIME_10s==0)){   //2015.1.31����3
	      if(FG_BAT==0){
		        FG_BAT=1;
			if(TIME_10s==0){      //2015.1.31����3
	...
	{												// Yes
		if	( m_KeyNew != d_KeyNoPush )		// Is key data no push ?
		{
                        if(((i==5)||(i==6)||(i==7)||(i==9)||(i==12))&&(FLAG_APP_TX==0));    //����������������δ��һֱ���� 
			else return ;								// No
		}
	}
	...
	switch	( i )    // Jumo to key function
	{
		...
		case 3 : 
		case 4 :   
		        if(FLAG_APP_TX==0)_FuncStop();   //2015.1.31����4
			break ;	
		case 5 :  
		case 6 :  
		case 7 :  
...
void	_Pass3secKey( uchar req )
{
	if	( m_TimerKey )								// 3sec passed ?
	{
		...
		{											// Yes
			_ReqTxdEdit( req,0 ) ;
                        FG_Complex_Single_shot=1;   //2015.1.31����2
			return ;
		}
	}
}
...
void	_FuncStop( void )
{
	...
	{												// No
		...
		{											// Yes
			if	(( !m_TimerKey )&&(FG_BAT==0)&&(TIME_Once_twice_switch==0))  //2015.1.31����4	  // 5sec passed ?
			{
//				if	( !--m_TimerKey )
//				{									// Yes
			...
			{										// Yes
				if	( m_KindOfKey == d_StopKey )
				{
					if	( m_KeyOptSetMode == 9 )
					{
						...
						rom_KeyOpt = m_KeyOpt;
                        IAP_WriteBuf_With_Protect_Verify(addr_eeprom_sys+Addr_rom_KeyOpt,&rom_KeyOpt,1);
						//KeyOpt_EEPROM_write();
