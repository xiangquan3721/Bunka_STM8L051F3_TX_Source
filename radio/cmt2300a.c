...
/*芯片驱动层源代码，提供芯片状态切换操作， 中断， GPIO， FIFO 操作， 通用寄存器配置和
访问等。 此部分属于固化的程序，和 MCU 平台无关，用户可不用对其修改*/
...
u8 CMT2300A_SwitchStatusAndWait(u8 nGoCmd, u8 nWaitStatus, u16 nEndTick)
{
    xdata u8 nCurStatus;

    Cmt_Spi_Write(CMT2300A_CUS_MODE_CTL, nGoCmd);

    while(CMT2300A_GetTickCount() < nEndTick)
    {
        nCurStatus = CMT2300A_GetChipStatus();

        if(nWaitStatus==nCurStatus || CMT2300A_STA_ERROR==nCurStatus)
            break;
    }
...
u8 CMT2300A_AutoSwitchStatus(u8 nGoCmd)
{
    ...
    while(CMT2300A_GetTickCount() < nEndTick)
    {
        ...
        if( ( (CMT2300A_GO_TFS==nGoCmd || CMT2300A_GO_TX==nGoCmd) && (CMT2300A_STA_RFS==nCurStatus || CMT2300A_STA_RX==nCurStatus) ) ||
            ( (CMT2300A_GO_RFS==nGoCmd || CMT2300A_GO_RX==nGoCmd) && (CMT2300A_STA_TFS==nCurStatus || CMT2300A_STA_TX==nCurStatus) )  )
        {
            nCurStatus = CMT2300A_SwitchStatusAndWait(CMT2300A_GO_STBY, CMT2300A_STA_STBY, nEndTick);
        }

        if(nCurStatus != nWaitStatus)
            nCurStatus = CMT2300A_SwitchStatusAndWait(nGoCmd, nWaitStatus, nEndTick);

        if(CMT2300A_STA_ERROR==nCurStatus)
            nCurStatus = CMT2300A_SwitchStatusAndWait(CMT2300A_GO_STBY, CMT2300A_STA_STBY, nEndTick);
...
{
    Cmt_Spi_Write(CMT2300A_CUS_IO_SEL, nGpioSel);
}
...
* *********************************************************/
void CMT2300A_SetInterruptPolar(u8 bActiveHigh)
{
    ...
    Cmt_Spi_Write(CMT2300A_CUS_INT1_CTL, tmp);
}
...
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_PKT29);

    tmp &= ~CMT2300A_MASK_FIFO_TH;
    tmp |= nFifoThreshold & CMT2300A_MASK_FIFO_TH;

    Cmt_Spi_Write(CMT2300A_CUS_PKT29, tmp);
}
...
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_INT1_CTL);
...
{
    Cmt_Spi_Write(CMT2300A_CUS_INT_EN, nEnable);
}
...
* *********************************************************/
void CMT2300A_EnableRxFifoAutoClear(u8 bEnable)
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_CTL);
    ...
    Cmt_Spi_Write(CMT2300A_CUS_FIFO_CTL, tmp);
}
...
* *********************************************************/
void CMT2300A_EnableFifoMerge(u8 bEnable)
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_CTL);
...
{
    Cmt_Spi_Write(CMT2300A_CUS_FIFO_CLR, CMT2300A_MASK_FIFO_RESTORE);
}
...
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_FLAG);
    Cmt_Spi_Write(CMT2300A_CUS_FIFO_CLR, CMT2300A_MASK_FIFO_CLR_TX);
    return tmp;
}
...
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_FLAG);
    Cmt_Spi_Write(CMT2300A_CUS_FIFO_CLR, CMT2300A_MASK_FIFO_CLR_RX);
    return tmp;
}
...
u8 CMT2300A_ClearInterruptFlags(void)
{
    ...
    Cmt_Spi_Write(CMT2300A_CUS_INT_CLR1, nClr1);
    Cmt_Spi_Write(CMT2300A_CUS_INT_CLR2, nClr2);
...
* *********************************************************/
void CMT2300A_EnableTxDin(u8 bEnable)
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_CTL);
...
* *********************************************************/
void CMT2300A_EnableTxDinInvert(u8 bEnable)
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_INT2_CTL);
    ...
    Cmt_Spi_Write(CMT2300A_CUS_INT2_CTL, tmp);
}
...
u8 CMT2300A_IsExist(void)
{
    xdata u8 back, dat;

    back = CMT2300A_ReadReg(CMT2300A_CUS_PKT17);
    Cmt_Spi_Write(CMT2300A_CUS_PKT17, 0xAA);

    dat = CMT2300A_ReadReg(CMT2300A_CUS_PKT17);
    Cmt_Spi_Write(CMT2300A_CUS_PKT17, back);
...
* @param   nChann: the frequency channel    (FREQ = 基础频点 + 2.5KHz * nOffset *  nChann)
* *********************************************************/
void CMT2300A_SetFrequencyChannel(u8 nChann)
{
    Cmt_Spi_Write(CMT2300A_CUS_FREQ_CHNL, nChann);
}
...
* @param   nOffset: the frequency step  (FREQ = 基础频点 + 2.5KHz * nOffset *  nChann)
* *********************************************************/
void CMT2300A_SetFrequencyStep(u8 nOffset)
{
    Cmt_Spi_Write(CMT2300A_CUS_FREQ_OFS, nOffset);
}
...
*          bEnable(FALSE): Disable it
* *********************************************************/
void CMT2300A_EnableLfosc(u8 bEnable)
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_SYS2);
    ...
    Cmt_Spi_Write(CMT2300A_CUS_SYS2, tmp);
}
...
* *********************************************************/
void CMT2300A_EnableLfoscOutput(u8 bEnable)
{
    xdata u8 tmp = CMT2300A_ReadReg(CMT2300A_CUS_INT2_CTL);
...
*          bEnable(FALSE): Disable it(default)
* *********************************************************/
void CMT2300A_EnableErrorStop(u8 bEnable)
{
    ...
    Cmt_Spi_Write(CMT2300A_CUS_EN_CTL, tmp);
}
...
void CMT2300A_Init(void)
{
    ...
    tmp |= CMT2300A_MASK_CFG_RETAIN;         /* Enable CFG_RETAIN *//*将CUS_MODE_STA（0x61）寄存器的第 4 个比特 CFG_RETAIN 配置成 1，
    这个比特会让 0x00 – 0x5F整个配置区的值不会被软复位擦除掉*/
    tmp &= ~CMT2300A_MASK_RSTN_IN_EN;        /* Disable RSTN_IN */
    Cmt_Spi_Write(CMT2300A_CUS_MODE_STA, tmp);

    CMT2300A_EnableLfosc(FALSE);             /* Diable LFOSC *///0不需要使用SLEEP TIMER
...
* *********************************************************/
u8 CMT2300A_ConfigRegBank(u8 base_addr, const u8 bank[], u8 len)
{
    xdata u8 i;
    for(i=0; i<len; i++)
        Cmt_Spi_Write(i+base_addr, bank[i]);
